...
The second condtion requires the contract to receive a message.
...
to do so we need information about the tranasaction and who signed it.

all the information about the tranasaction is in the `tx` field of the [`PScriptContex`](../onchain/API/types/PScriptContex)
